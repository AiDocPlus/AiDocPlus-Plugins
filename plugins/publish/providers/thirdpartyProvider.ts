...
function apiUrl(config: ThirdPartyConfig, path: string): string {
  const base = config.providerUrl.replace(/\/+$/, '');
  return `${base}${path}`;
}

export function createThirdPartyProvider(host: PluginHostAPI, config: ThirdPartyConfig): WechatApiProvider {
  return {
    mode: 'thirdparty',
    label: config.providerName || '第三方服务',
